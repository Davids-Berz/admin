...
public class TemplateStandardMotor extends SiteEnginePass {
   ...
   private static final String SITE_NAME = "MX COSTCO TEST"; // TODO
   ...
   private static final DateTimeFormatter DELIVERY_DATE_PARSER = DateTimeFormat.forPattern("dd-MM-yyyy").withLocale(CURRENT_LOCALE); // TODO
   private static final DateTimeFormatter REVIEW_DATE_PARSER = DateTimeFormat.forPattern("dd/MM/yyyy").withLocale(CURRENT_LOCALE); // TODO
   ...
   private void logSegmentProperties() {
      conn.debug("...........................");
      debug("Segment Id", segment.getId());
      debug("Segment Rpid", segment.getRpid());
      debug("Segment Path", getPathSegment());
      debug("Segment Url", segment.getUrl());
      debug("Country", country);
      debug("Proxy", proxy);
      debug("Port", proxyPort);
      conn.debug("...........................");
   }
   ...
   private void configureEngine() {
      ...
      country = propertyService.getPropertyValue(String.format(MOTOR_SITE_COUNTRY, segment.getSid()));
   ...
   private void configureSite() {
      try {
         ...
         urlConsolidator = new UrlConsolidator(protocol + "://" + url.getHost() + "/");
   ...
   private void paginateThroughSegments() {
      ...
      try {
         ...
         while (continueCrawl) {
            ...
            debug("Page", loop);
            debug("Next page URL", request.getUrl());
   ...
   private void logStatistics() {
      showResult("Offers crawled and saved : " + savedCount);
   ...
   private void saveSiteProduct(SiteProduct siteProduct) {
      if (conn.addProduct(siteProduct)){
   ...
   private void addBrandToTitle(SiteProduct siteProduct) {
      if (isNotBlank(siteProduct.getBrand()) && !containsIgnoreCase(siteProduct.getTitle(), siteProduct.getBrand())) {
         String title = Joiner.on(" ").skipNulls().join(Strings.emptyToNull(siteProduct.getBrand()), siteProduct.getTitle());
         debug("Title with brand", title);
         siteProduct.setTitle(title);
      }
   }
   ...
   private SiteProduct handleProductPage(final SiteProduct siteProduct) {
      ...
      // Response productPage = connection(post(url, "%Form data%"), "product page");// TODO
      ...
      debug("Previous price from site", previousPriceFromSite);
      ...
      debug("Availability text", availabilityText);
      ...
      debug("Raw delivery", rawDelivery);
      ...
      debug("Delivery price home", deliveryPriceHome);
   ...
   private void handleRatingAndReviews(Document productPageDocument, SiteProduct siteProduct) {
      ...
      debug("Rating total", ratingTotal);
   ...
   private void handleReviews(final Element element, SiteProduct siteProduct) {
      ...
      for (Element reviewElement : reviewsElements) {
         ...
         debug("Review Rating", reviewRating);
   ...
   private void handleRatingBands(final Element element, SiteProduct siteProduct) {
      ...
      debug("Rating band 0 to 20", ratingBand0to20);
      ...
      debug("Rating band 20 to 40", ratingBand20to40);
      ...
      debug("Rating band 40 to 60", ratingBand40to60);
      ...
      debug("Rating band 60 to 80", ratingBand60to80);
      ...
      debug("Rating band 80 to 100", ratingBand80to100);
   ...
   private Integer getRatingBand(final Element element, final int stars, final String url) {
      final Element ratingBandElement = findElement(element, "Rating band selector ...");// TODO Ex : "div.ratingBand[stars=" + stars + "]
   ...
   private int getDelivery(final String rawDelivery, final String url) {
      if (isNotBlank(rawDelivery)) {
         final String lcRawDelivery = lowerCase(rawDelivery);
   ...
   private String checkEAN(final String ean) {
      ...
      if (exactEan.length() > 10) {
         exactEan = leftPad(exactEan, 13, CharUtils.toChar("0"));
      }
      final int length = exactEan.length();
      if (length > 13) {
         return substring(exactEan, length - 13, length);
      }
      return exactEan;
   }

   private String getMPN(final Element element, final String url) {
      Element mpnElement = findElement(element, "tr:contains(Modelo) td.attrib-val");// TODO
   ...
   private String isValidMPN(final String mpn) {
      if (isNotBlank(mpn)) {
          ...
          *
          */
         if (isNumericSpace(mpn)) {
            conn.debug("MPN consists only of digits and/or '-' or EMPTY [" + mpn + "]. Set MPN to null!");
            return null;
         }
         if (isAlphaSpace(mpn)) {
            conn.debug("MPN consists only of letters and/or '-' or EMPTY [" + mpn + "]. Set MPN to null!");
            return null;
         }
         if (countMatches(mpn, SPACE) > 1) {
         ...
         return deleteWhitespace(mpn);
      }
      return null;
   }

   private String getSKU(final Element element, final String url) {
      final Element skuElement = findElement(element, ".hidden-xs .product-code span");// TODO
   ...
   private String getBrand(final Element element, final String url) {
      final Element brandElement = findElement(element, "tr:contains(Marca) td.attrib-val");// TODO
   ...
   private boolean getAvailability(final String availabilityText, final String url) {
      if (startsWithAny(availabilityText, "InStock", "LimitedStock", "PreOrder")) {// TODO
         return true;
      }
      if (startsWithAny(availabilityText, "OutOfStock", "SoldOut", "BackOrder")) {// TODO
         return false;
      }
   ...
   private float getPrice(final Element element, final String url) {
      final Element priceElement = findElement(element, ".price-original span.notranslate");// TODO
      String priceRaw = fromElementText(priceElement);
   ...
   private PageResponse connection(final PageRequest request, final String type) {
      debug("Connect on " + type, request.getUrl());
      try {
...
//      httpGet.addHeader("Accept-Language", "es-MX,es-419;q=0.9,es;q=0.8"); // TODO
      ...
      httpGet.addHeader("User-Agent", "Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/70.0.3538.102 Safari/537.36");
   ...
   private HttpPost createHttpPost(HttpParams params, PageRequest request) throws UnsupportedEncodingException {
      ...
      httpPost.addHeader("Host", getHost(request.getUrl()));
      httpPost.addHeader("User-Agent", "Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/70.0.3538.102 Safari/537.36");
      httpPost.addHeader("X-Requested-With", "XMLHttpRequest");
      httpPost.setEntity(new StringEntity(request.getEntity(), StandardCharsets.UTF_8));
   ...
   private final ResponseHandler<PageResponse> responseHandler = new ResponseHandler<PageResponse>() {
      ...
      public PageResponse handleResponse(HttpResponse httpResponse) throws IOException {
         ...
         debug("Status", pageResponse.getStatusCode());

         pageResponse.setSetCookie(getCookie(httpResponse.getHeaders("Set-Cookie")));
         debug("Set-Cookie", pageResponse.getSetCookie());
      ...
      private String getContent(HttpResponse response) throws IOException {
         ...
         if (contentEncodingHeader != null && contentEncodingHeader.getValue().contains("gzip")) {
            return trim(IOUtils.toString(new GzipDecompressingEntity(response.getEntity()).getContent(), HTTP.UTF_8));
         }
      ...
      private String getCookie(Header... headers) {
         ...
         for (Header header : headers) {
            list.add(substringBefore(header.getValue(), ";"));
         }
   ...
   private void showReview(String message) {
      conn.debug("<div style='color: white; background-color: maroon; text-align: center'>" + message + "</div>");
   }

   private void showOffer(final String message) {
      conn.debug("<div style='color: white; background-color: navy; text-align: center'>" + message + "</div>");
   }

   private void showResult(final String message) {
      conn.debug("<div style='color: white; background-color: green; text-align: center'>" + message + "</div>");
   }

   private void debug(String label, Object value) {
      if (isBlank(label)) return;
      label = rightPad(label, 24, CARACTERE_ESPACE);
      conn.debug(label + " : " + value);
   }

   private void showCode(final Object object) {
      if (object == null) {
         conn.warn("Object null => nothing to show");
         return;
      }

      String code = object.toString();
      conn.debug("*************************BEGIN CODE*************************");
      conn.debug(code.replace("<", "&" + "lt" + ";").replace(">", "&" + "gt" + ";"));
      conn.debug("**************************END CODE**************************");
   }
}
